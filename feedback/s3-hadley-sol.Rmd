...
new_data.frame <- function(x = list(), row.names = character()) {
  ...
  # NB: by default, base-R would turn all character vectors in data into factors
  # at this point, but that sucks anyway so we don't....
  structure(x, 
            row.names = row.names,
            class = "data.frame")
}
...
validate_data.frame <- function(x) {
  ...
  # and can be anything except functions, environments or NULL:
  checkmate::assert_list(data, 
                         names = "strict",
                         types = c("atomic", "factor", "matrix", "list")) 
  # dataframe columns must have identical lengths. 
  # for arrays/matrices, this is their first dimension,
  # for lists and vectors, it's their length:
  ...
  if (!length(data) == 0) { # edge case: empty data.frame without columns
    nrows <- vapply(data, get_nrows, numeric(1))
...
Da `class(a)` `"test"` ist, wird also eine Methode (Funktion) namens `t.test` gesucht. Diese wird gefunden und mit Argument `a` aufgerufen. `t.test()` ist aber selbst eine *generic*, keine Methode, triggert also nochmal einen *method dispatch*, gesucht wird jetzt aber die Funktion `t.test.test()` (!!). Die gibt es nüscht, also wird `t.test.default` benutzt, was in dem Fall hier testet ob der Mittelwert von `a` O ist.
